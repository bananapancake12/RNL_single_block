...
!  and send this data to the other procs.
  if (myid==0) then
    ...
    ! if (geometry_type == 0) then
      ! post_spacing = 0d0                        ! dont think this is necessary...
      ! posth        =   0d0
      ! h_ny(1)      = .25d0  
...
!   ! Toggle for guarded minimal startup: set to .true. to stop after ygrid
!   do_minimal_start = .false.
...
!   ! ------------------------------------------------------------------
!   ! Temporary debug guard: stop here to perform minimal startup checks.
!   ! Toggle `do_minimal_start` at the top of this routine to enable/disable.
!   if (do_minimal_start) then
!     write(*,*) 'DEBUG: minimal startup complete on proc', myid
!     return
!   end if
!   ! End temporary debug guard
...
subroutine proc_lims_columns(myid)
  ...
  ! columns_num_long  = (N(1,nband)/2 + 1) * (N(2,nband)) !*
  ...
  ! write(*,*) 'columns_num_total_proc      = ', columns_num_total_proc
  ! write(*,*) 'columns_num_total_proc_rem  = ', columns_num_total_proc_rem
  ...
  !   columns_num(iproc) = columns_num_short_proc +1
  !   columns_num(iproc) = columns_num_short_proc +1
  ! end do
  ...
  !   columns_num(iproc) = columns_num_short_proc
  !   columns_num(iproc) = columns_num_short_proc
  ! end do
